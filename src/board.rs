...
impl BoardState{
    ...
    pub fn try_to_move(& mut self, mut position_list: Vec<String>, red_turn: bool) -> Result<String, String> {
        ...
        while position_list.len() > 0 {
            if moved_yet && taken_piece_posns.len() == 0 {
                break;
            }
            ...
            match self.is_valid_move(curr_row, curr_col, next_row, next_col, red_turn, moved_yet) {
                Ok(valid) => if valid {
        ...
        if position_list.len() > 0 {
            return Err("Cannot move 1 space after any other move.".to_owned());
        }
    ...
    fn is_valid_move(&self, curr_row: u32, curr_col: u32, next_row: u32, 
                     next_col: u32, red_turn: bool, moved_yet: bool) -> Result<bool, String> {
        ...
        if red_turn {
            match curr_tile {
                Tile::Empty => {
                    if !moved_yet {
                        return Err("Target position".to_owned() + " is empty.")
                    }
                },
        ...
        if is_royal {
            ...
            if red_turn {
                if (curr_row as i32 - next_row as i32) == -2 &&
                   (curr_col as i32 - next_col as i32).abs() == 2 {
                    let tile_to_take = self.get_tile_at(
                    (curr_row as i32 - (curr_row as i32 - next_row as i32) / 2) as u32,
                    (curr_col as i32 - (curr_col as i32 - next_col as i32) / 2) as u32)?;

                    return Ok(tile_to_take == &Tile::Black || tile_to_take == &Tile::BlackRoyals);
                }
                else {
                    return Ok((curr_row as i32 - next_row as i32) == -1 &&
                            (curr_col as i32 - next_col as i32).abs() == 1 && !moved_yet) 
                }
            }
            else {
                if (curr_row as i32 - next_row as i32) == 2 &&
    ...
    fn get_taken_piece(&self, curr_row: u32, curr_col: u32, next_row: u32,
        ...
        if (curr_row as i32 - next_row as i32).abs() == 2 && 
               (curr_col as i32 - next_col as i32).abs() == 2 {
            return Some(((curr_row as i32 - (curr_row as i32 - next_row as i32) / 2) as u32,
                    (curr_col as i32 - (curr_col as i32 - next_col as i32) / 2) as u32))        
        }
        return None;
    }
